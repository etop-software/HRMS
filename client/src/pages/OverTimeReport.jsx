      ...
      <Document>
        <Page style={styles.page}>
          <View style={styles.section}>
            <Text style={styles.title}>Report Type: Overtime</Text>
            ...
            <View style={styles.table}>
              ...
              <View style={styles.tableHeaderRow}>
                ...
                <Text style={[styles.tableHeaderCell, styles.columnLateHrs]}>OT Hrs</Text>
              </View>
              ...
              {reportData.map((row, index) => (
                <View key={index} style={styles.tableRow}>
                  ...
                  <Text style={[styles.tableCell, styles.columnFlex]}>{row.total_worked_hours || "-"}</Text>
                  <Text style={[styles.tableCell, styles.columnLateHrs]}>{row.overtime_formatted || "-"}</Text>
                </View>
              ))}
            </View>
          </View>
  ...
  const fetchReport = async () => {
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/Reports/OverTime-report`,
  ...
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(
      reportData.map((row) => ({
        ...
        "Total Hours": row.total_worked_hours || "-",
        "OT Hrs": row.overtime_formatted || "-",
      }))
    );

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Overtime Report");
    XLSX.writeFile(wb, "Overtime_Report.xlsx");
  };
    ...
    <div className="max-w-7xl mx-auto">
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
              <div className="flex items-center">
                ...
                <span className="ml-1 text-gray-500 md:ml-2">Overtime Report</span>
              </div>
            </li>
          </ol>
        </nav>
      ...
      <div className="bg-white rounded-lg shadow-md p-3 mb-3">
        <div className="flex space-x-6">
          ...
          <PDFDownloadLink document={<MyDocument />} fileName="Overtime_Report.pdf">
            {({ loading }) =>
              loading ? (
    ...
    <table className="min-w-full bg-white border-collapse table-auto text-sm cursor-pointer">
      <thead className="bg-gray-100">
        <tr>
          ...
          <th className="px-3 text-nowrap text-left font-medium text-gray-700">OT Hrs</th>
        </tr>
      </thead>
      <tbody>
        {reportData
          .filter(row => row.employee_name.toLowerCase().includes(searchTerm.toLowerCase()))
          .map((row, index) => (
            <tr key={index} className="border-b hover:bg-gray-50 transition duration-200 cursor-pointer">
              <td className="px-5 py-2 text-nowrap text-left">{row.employee_id}</td>
              <td className="px-5 py-2 text-nowrap text-left">{row.employee_name}</td>
              <td className="px-4 text-nowrap text-left">{row.work_date.split("T")[0]  || "-"}</td>
              <td className="px-4 text-nowrap text-left">{row.total_worked_hours || "-"}</td>
              <td className="px-4 text-nowrap text-left">{row.overtime_formatted || "-"}</td>
            </tr>
          ))}

        {reportData.filter(row => row.employee_name.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 && (
          <tr>
