...
WITH daily_totals AS ( SELECT a.employee_id, e.name, DATE(a.datetime) AS work_day, EXTRACT(EPOCH FROM ( MAX(CASE WHEN a.attendance_state = '1' THEN a.datetime END) - MIN(CASE WHEN a.attendance_state = '0' THEN a.datetime END) )) / 3600 AS daily_hours FROM employees e JOIN attendance a ON e.employee_id = a.employee_id WHERE EXTRACT(MONTH FROM a.datetime) = MM AND EXTRACT(YEAR FROM a.datetime) = YYYY GROUP BY a.employee_id, e.name, DATE(a.datetime) ) SELECT dt.employee_id, dt.name, TO_CHAR(INTERVAL '1 hour' * MAX(CASE WHEN dt.work_day = 'YYYY-MM-01' THEN dt.daily_hours END), 'HH24:MI') AS "01/MM/YYYY", TO_CHAR(INTERVAL '1 hour' * MAX(CASE WHEN dt.work_day = 'YYYY-MM-02' THEN dt.daily_hours END), 'HH24:MI') AS "02/MM/YYYY", ... TO_CHAR(INTERVAL '1 hour' * MAX(CASE WHEN dt.work_day = 'YYYY-MM-DD' THEN dt.daily_hours END), 'HH24:MI') AS "DD/MM/YYYY", TO_CHAR( INTERVAL '1 hour' * COALESCE(SUM(dt.daily_hours), 0), 'HH24:MI' ) AS total_hours FROM daily_totals dt GROUP BY dt.employee_id, dt.name ORDER BY dt.employee_id;
Replace:
...
Generate TO_CHAR(INTERVAL '1 hour' * MAX(CASE ...), 'HH24:MI') clauses for each day of the month (01 to DD).
...
Use the following pattern for specific day reports:
WITH daily_totals AS ( SELECT a.employee_id, e.name, DATE(a.datetime) AS work_day, EXTRACT(EPOCH FROM ( MAX(CASE WHEN a.attendance_state = '1' THEN a.datetime END) - MIN(CASE WHEN a.attendance_state = '0' THEN a.datetime END) )) / 3600 AS daily_hours FROM employees e JOIN attendance a ON e.employee_id = a.employee_id WHERE DATE(a.datetime) = 'YYYY-MM-DD' GROUP BY a.employee_id, e.name, DATE(a.datetime) ) SELECT dt.employee_id, dt.name, TO_CHAR(INTERVAL '1 hour' * COALESCE(dt.daily_hours, 0), 'HH24:MI') AS hours_worked FROM daily_totals dt ORDER BY dt.employee_id;
Replace:
...
Do NOT calculate working hours using MIN/MAX across the date range.
Instead, calculate working hours per employee per day using:
...
Then, aggregate daily totals to monthly:
...
### Important:
Return total_hours_in_month in HH:MM format instead of float.
...
- Format as zero-padded HH:MM
  ...
  Map natural language terms to standard leave types as follows:
  ...
  - Determine the mapped leave type based on the user's wording
  - Use ILIKE and include all relevant synonyms in a grouped condition
  
  Example:
  If the user says "vacation", generate:
  ...
  WHERE l.leave_name ILIKE '%annual%' OR l.leave_name ILIKE '%vacation%';
  ...
  FROM leaves l
  JOIN leave_policies lp ON l.id = lp.leave_type_id
  WHERE l.leave_name ILIKE '%personal%' OR l.leave_name ILIKE '%casual%';
